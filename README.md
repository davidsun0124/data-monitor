...
3. **填 `false` 即彻底关闭**：如果你想完全关闭某个功能或不设限制，请显式填写 `false`（或 `none`、`null`、`""`），调度器会自动处理。
   - 例如：`schedule: false` 会关闭定时触发，效果等同于 `schedule: manual`。
   - 例如：`db_host: false` 会禁用数据库注入功能。
...
A: 说明 AI 思考次数或资源消耗 (Tokens) 超过了限制。请在对应任务的 `.md` 头部调大 `max_turns` 或 `budget`。
...
2. 增加OWASP Top 10扫描+敏感信息检测 安全扫描, 并输入安全扫描报告.--已增加到对应分支
